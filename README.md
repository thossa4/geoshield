...
| 2 — Data governance / source registry | `docs/data_registry.csv` — all 10 rows live-verified (no `TODO`s): FEMA NFHL, FEMA NRI, USGS 3DEP, USGS NLCD, NOAA NCEI, USGS NHD, and the East Baton Rouge Parish GIS stormwater datasets are wired into working modules; Census ACS is built and live-verified (with a founder-provided key) but deliberately not wired into the customer report yet (blueprint scopes it as optional/B2B, and no claims_matrix.md entry exists for it); LA Fortify Homes and LA GOHSEP are verified-live program pages, not indicator data |
| 3 — Geocoding | Working: `src/geocoding/geocode_address.py` (US Census Geocoder, live) |
